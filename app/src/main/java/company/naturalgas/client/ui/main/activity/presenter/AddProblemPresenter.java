...
public class AddProblemPresenter extends BaseMvp_Presenter<AddProblemAct_V>
{
    ...
    public void uploadDanger(String transmitDealId,String description,List<String> filesPath)
    {
        if(isAttachContextAndViewLayer())
        {
            BaseMvp_EntranceOfModel.requestDatas(AddProblemModel.class).putForm("transmitDealId",transmitDealId).putForm("description",description).convertForms().
            putImagesPath(filesPath).executeOfNet(getContext(),AddProblemModel.UpLoadDanger,new BaseMvp_LocalObjCallBack<BaseReturnData>(this)
                    {
